...
\begin{algorithm}
\caption{Gradient Boosted Regression Trees (GBRT). DT means the decision tree model which has three parameters, data $D$, #features\mbox{ }f and the depth $d$ of tree.}\label{algo:gbrt}
\begin{algorithmic}
...
\vpara{Evaluation Measures} To quantitatively evaluate the performance of predicting the degree distribution, we conducted experiments with different networks, for each network we and evaluated the approaches in terms of \textbf{Average Relative Error (ARE)} by using the maximum.

\vpara{Prediction Performance} We use 168 different network as data set to evaluate the proposed model. We use 10-fold cross validation to \textbf{Average Relative Error (ARE)}. In order to valid the bias, we test the data 10 times, and get the prediction alpha for each network. Finally, the model achieves $0.005521 \pm 0.00316$.
